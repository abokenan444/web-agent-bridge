...
(function (window, document) {
  ...
  function applyResult(badge, tooltip, result, url) {
    ...
    tooltip.innerHTML = `
      ...
      <div class="wab-tooltip-score">Risk Score: ${risk_score}/100 · 47 databases checked</div>
      <div class="wab-tooltip-footer">Powered by <a href="https://www.webagentbridge.com" target="_blank">WAB — Web Agent Bridge</a></div>
    `;
  }
  ...
  function showBlockOverlay(url, verdict, onProceed) {
    ...
    overlay.innerHTML = `
      <div class="wab-blocked-box">
        ...
        <div class="wab-blocked-msg">${verdict || 'This link has been identified as malicious by WAB Scam Shield.'}</div>
        ...
        <div class="wab-blocked-powered">Protected by <a href="https://www.webagentbridge.com" target="_blank">WAB — Web Agent Bridge</a></div>
      </div>
    `;
  ...
  function processLinks(root = document) {
    ...
    links.forEach(link => {
      ...
      scanURL(url).then(result => {
        ...
        // Block mode: intercept click on dangerous links
        if (MODE === 'block' && result.status === 'CRITICAL') {
          link.addEventListener('click', e => {
            e.preventDefault();
            showBlockOverlay(url, result.verdict, () => window.open(url, '_blank'));
          });
        }
      });
    });
    ...
    const observer = new MutationObserver(mutations => {
      mutations.forEach(m => {
        m.addedNodes.forEach(node => {
          if (node.nodeType === 1) {
            if (node.tagName === 'A') processLinks(node.parentElement);
            else if (node.querySelectorAll) processLinks(node);
          }
        });
      });
    });
  ...
  function init() {
    if (!API_KEY) {
      console.warn('[WAB Widget] No API key found. Add data-wab-key="YOUR_KEY" to the script tag. Get yours at https://www.webagentbridge.com/workspace');
      return;
    }
    injectStyles();
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => { processLinks(); observeDOM(); });
    } else {
      processLinks();
      observeDOM();
    }
    console.info(`[WAB Widget v${WAB_VER}] Initialized in "${MODE}" mode. Powered by https://www.webagentbridge.com`);
  }

  init();

  // Public API
  window.WABWidget = { scan: scanURL, processLinks, version: WAB_VER };
