'use strict';
/**
 * WAB Platform Demo — Backend API Server
 * Isolated demo environment — no production DB connections
 * All responses are realistic simulations of the real WAB platform
 */
...
app.use((req, res, next) => {
  ...
  if (entry.count > maxReqs) return res.status(429).json({ error: 'Rate limit exceeded. Please wait.' });
  next();
});
...
app.get('/api/health', (req, res) => {
  ...
  res.json({
    ...
    version: '2.5.0-demo',
    isolation: 'full — no production DB connections',
...
const fairnessDatabase = {
  ...
  'booking.com': {
    ...
    recommendation: '⚠️ Booking.com charges hotels 15–25% commission, which is passed on to you. Always check the hotel\'s direct website after finding it on Booking.com — direct rates are often 10–18% cheaper and include free cancellation. WAB can automate this comparison for you.',
  },
  'etsy.com': {
    ...
    recommendation: '✅ Etsy scores well on fairness. The 6.5% transaction fee is reasonable for the handmade marketplace. Sellers are relatively transparent and the platform supports direct communication. A solid choice for unique, handcrafted items.',
  },
  'ebay.com': {
    ...
    recommendation: '🔵 eBay offers reasonable fairness, especially for used goods and auctions. The 10–12% final value fee is moderate. Watch out for shipping costs that aren\'t shown upfront. For new items, compare with direct manufacturer pricing.',
  },
  'airbnb.com': {
    ...
    recommendation: '❌ Airbnb has significant fairness issues. The advertised nightly price is rarely the final price — cleaning fees ($50–$200) and service fees (14–16%) can double the cost. Always click "Show total price" before comparing. Consider VRBO or direct host contact for longer stays.',
  },
};
...
function generateFairnessForUnknown(domain) {
  ...
  return {
    ...
    platformSize: 'Unknown',
    overall: base,
    verdict: base >= 75 ? 'Good' : base >= 55 ? 'Moderate' : 'Poor',
    ...
    breakdown: {
      ...
      priceHonesty: { label: 'Price Honesty', score: Math.max(10, base - 8), description: 'Limited data available for this domain' },
      ...
      directBooking: { label: 'Direct Access', score: Math.min(90, base + 20), description: 'Direct domain — no intermediary detected' },
      hiddenFees: { label: 'Hidden Fee Score', score: Math.max(15, base - 3), description: 'Requires deeper checkout analysis' },
    },
    recommendation: `ℹ️ Limited data available for ${domain}. The WAB platform has analyzed the domain structure and SSL configuration. For a complete fairness assessment, the platform would need to crawl the checkout flow. This is a demo simulation — real WAB scores are based on live crawl data.`,
  };
}

app.post('/api/demo/fairness', async (req, res) => {
  const { url } = req.body;
  if (!url) return res.status(400).json({ error: 'url is required' });

  await sleep(1200 + Math.random() * 800);
  const domain = extractDomain(url);
  const result = fairnessDatabase[domain] || generateFairnessForUnknown(domain);
  demoStats.requestsServed++;
  res.json(result);
});
...
function generateShieldResult(domain, riskScore, riskLevel) {
  ...
  const checks = [
    {
      name: 'Domain Age',
      passed: isSafe || (!isHigh && domain.length > 8),
      detail: isSafe ? 'Domain registered 8+ years ago — established' : isHigh ? 'Domain registered < 30 days ago — suspicious' : 'Domain registered 6 months ago — monitor',
    },
    {
      name: 'SSL Certificate',
      passed: isSafe || isMedium,
      detail: isSafe ? 'Valid EV certificate from trusted CA' : isHigh ? 'Self-signed or expired certificate' : 'Valid DV certificate — basic security',
    },
    {
      name: 'Phishing Databases',
      passed: isSafe,
      detail: isSafe ? 'Not found in any of 47 phishing databases' : isHigh ? `Listed in ${Math.floor(riskScore / 10)} phishing databases` : 'Flagged in 2 monitoring lists',
    },
    {
      name: 'Homograph Detection',
      passed: isSafe || isMedium,
      detail: isSafe ? 'No lookalike characters detected in domain' : isHigh ? 'Homograph attack detected: uses "0" instead of "o"' : 'No homograph characters found',
    },
    {
      name: 'Reputation Score',
      passed: isSafe,
      detail: isSafe ? 'Excellent reputation across all sources' : isHigh ? 'Negative reputation — reported by users' : 'Neutral reputation — insufficient data',
    },
    {
      name: 'Malware Scan',
      passed: isSafe || isMedium,
      detail: isSafe ? 'No malware or malicious scripts detected' : isHigh ? 'Credential harvesting script detected' : 'No malware detected — clean',
    },
  ];

  const recommendations = {
    safe: `✅ **${domain}** is safe to visit. Our analysis across 47 security databases found no threats. SSL certificate is valid and the domain has a strong reputation. WAB Agent can safely interact with this site.`,
    medium: `⚠️ **${domain}** shows some suspicious signals. The domain is relatively new and has limited reputation data. Proceed with caution — avoid entering sensitive information until more data is available. WAB Scam Shield will monitor interactions.`,
    high: `🚨 **DANGER: ${domain}** is a known threat. This domain has been flagged in multiple phishing databases and shows signs of a homograph attack (using look-alike characters). **Do not visit this site.** WAB Agent has blocked all interactions with this domain.`,
    critical: `🚨 **CRITICAL THREAT: ${domain}** is an active phishing site. Immediately flagged across ${Math.floor(riskScore / 10)} security databases. This site is impersonating a legitimate service to steal credentials. WAB Scam Shield has blocked this URL and reported it to security authorities.`,
  };
  ...
  return {
    ...
    riskLevel,
    checks,
    recommendation: recommendations[riskLevel] || recommendations.medium,
    databasesChecked: 47,
    scanTime: `${120 + Math.floor(Math.random() * 80)}ms`,
  };
}

app.post('/api/demo/shield', async (req, res) => {
  const { url } = req.body;
  if (!url) return res.status(400).json({ error: 'url is required' });
...
const dealsDatabase = {
  laptop: [
    ...
    { platform: 'eBay (New)', domain: 'ebay.com', price: 461.00, originalPrice: 549.99, savings: '16%', fairness: 64, badge: null, hiddenFees: '⚠️ $12 shipping + 10% final value fee to seller' },
  ],
  'wireless headphones': [
    ...
    { platform: 'B&H Photo', domain: 'bhphotovideo.com', price: 289.95, originalPrice: 349.99, savings: '17%', fairness: 82, badge: 'Good Deal', hiddenFees: 'Free expedited shipping — no hidden fees' },
    { platform: 'Amazon', domain: 'amazon.com', price: 319.99, originalPrice: 349.99, savings: '9%', fairness: 58, badge: null, hiddenFees: '⚠️ 8% commission embedded — check direct price' },
  ...
  'mystery books': [
    { platform: 'Author Direct (Bookshop.org)', domain: 'bookshop.org', price: 14.99, originalPrice: 18.99, savings: '21%', fairness: 94, badge: 'Best Value', hiddenFees: 'Supports local bookstores — no hidden fees' },
  ...
  'running shoes': [
    { platform: 'Nike Official', domain: 'nike.com', price: 89.99, originalPrice: 110.00, savings: '18%', fairness: 85, badge: 'Best Value', hiddenFees: 'Free shipping + free returns — no fees' },
    ...
    { platform: 'Running Warehouse', domain: 'runningwarehouse.com', price: 94.95, originalPrice: 110.00, savings: '14%', fairness: 88, badge: 'Good Deal', hiddenFees: 'Free 2-day shipping — specialist retailer' },
    { platform: 'Zappos', domain: 'zappos.com', price: 99.99, originalPrice: 110.00, savings: '9%', fairness: 79, badge: null, hiddenFees: 'Free shipping + free returns — fair pricing' },
...
function generateDealsForQuery(query) {
  ...
  return [
    { platform: 'Direct Manufacturer', domain: `${query.replace(/\s+/g, '')}.com`, price: basePrice * 0.85, originalPrice: basePrice, savings: '15%', fairness: 88, badge: 'Best Value', hiddenFees: 'No hidden fees — direct purchase' },
    { platform: 'Specialty Retailer', domain: 'specialty-store.com', price: basePrice * 0.90, originalPrice: basePrice, savings: '10%', fairness: 79, badge: 'Good Deal', hiddenFees: 'Free shipping over $50' },
    { platform: 'Amazon', domain: 'amazon.com', price: basePrice * 0.95, originalPrice: basePrice, savings: '5%', fairness: 58, badge: null, hiddenFees: '⚠️ Commission embedded in price' },
    { platform: 'eBay', domain: 'ebay.com', price: basePrice * 0.88, originalPrice: basePrice, savings: '12%', fairness: 64, badge: null, hiddenFees: 'Shipping costs vary by seller' },
  ];
}

app.post('/api/demo/deals', async (req, res) => {
  ...
  const cheapest = deals[0].price;
  const mostExpensive = deals[deals.length - 1].price;
  ...
  res.json({
    ...
    deals,
    savings: {
      amount: (mostExpensive - cheapest).toFixed(2),
      percentage: `${savingsPct}%`,
    },
    scanTime: `${1500 + Math.floor(Math.random() * 500)}ms`,
    sitesScanned: 50000,
  });
});

// ─── Architecture Info ────────────────────────────────────────────────────────
app.get('/api/demo/architecture', (req, res) => {
  res.json({
    layers: [
    ...
    fairnessSignals: 15,
    securityDatabases: 47,
    supportedSites: 50000,
    dailyActions: 2400000,
    uptime: '99.97%',
    avgLatency: '142ms',
  });
});
...
// ─── 404 Handler ──────────────────────────────────────────────────────────────
app.use((req, res) => {
  res.status(404).json({ error: 'Endpoint not found', available: ['/api/health', '/api/demo/stats', '/api/demo/agent', '/api/demo/fairness', '/api/demo/shield', '/api/demo/deals', '/api/demo/architecture'] });
});

// ─── Start Server ─────────────────────────────────────────────────────────────
app.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ WAB Demo Backend running on port ${PORT}`);
  console.log(`🔒 Environment: DEMO (isolated — no production DB)`);
