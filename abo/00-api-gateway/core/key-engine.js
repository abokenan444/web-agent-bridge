/**
 * WAB API Key Engine
 * The core authentication and authorization layer for all WAB modules.
 * Handles key generation, validation, plan enforcement, rate limiting,
 * usage tracking, and quota management.
 *
 * Architecture: All 10 WAB modules are HIDDEN behind this gateway.
 * External world only sees: api.webagentbridge.com
 * Internal modules are never exposed directly.
 *
...
const PLANS = {
  FREE: {
    ...
    modules_allowed: ['scam-shield', 'fairness-check', 'price-history', 'dark-pattern', 'price', 'protocol', 'bounty'],
    features: {
      agent_firewall: false,
      notary: false,
      dark_pattern: true,
      collective_bargaining: false,
      gov_intelligence: false,
      price_time_machine: true,
      neural: false,
      protocol: true,
      bounty: true,
      affiliate: false,
  ...
  PRO: {
    ...
    modules_allowed: ['scam-shield', 'fairness-check', 'price-history', 'agent-firewall', 'dark-pattern', 'neural', 'bounty', 'affiliate', 'protocol'],
    features: {
      agent_firewall: true,
      notary: false,
      dark_pattern: true,
...
// ─── In-memory Key Store (replace with Redis/PostgreSQL in production) ─────────
const keyStore = new Map();          // apiKey → KeyRecord
const usageStore = new Map();        // apiKey → UsageRecord
const rateLimitStore = new Map();    // apiKey → { count, windowStart }

// ─── Seed internal keys ───────────────────────────────────────────────────────
function seedInternalKeys() {
  const internalKey = 'wab_internal_' + crypto.randomBytes(16).toString('hex');
  keyStore.set(internalKey, {
    key: internalKey,
    key_id: 'kid_internal_001',
    plan: 'INTERNAL',
    owner: 'WAB Core Team',
    email: 'dev@webagentbridge.com',
    environment: 'internal',
    created_at: new Date().toISOString(),
    last_used: null,
    active: true,
    scopes: ['*'],
    metadata: { team: 'core', purpose: 'development' },
  });
  console.log(`[WAB Key Engine] Internal key seeded: ${internalKey.substring(0, 20)}...`);
  return internalKey;
}
...
class WABKeyEngine {
  constructor() {
    this.internalKey = seedInternalKeys();
  }

  /**
   * Generate a new API key
   * Format: wab_{env}_{plan_prefix}_{random32hex}
   * Example: wab_live_pro_a1b2c3d4e5f6...
   */
  generateKey(options = {}) {
    const {
      plan = 'FREE',
    ...
    if (!PLANS[plan]) throw new Error(`Invalid plan: ${plan}. Valid: ${Object.keys(PLANS).join(', ')}`);
    ...
    // Generate a secret hash for webhook verification
    ...
    const keyRecord = {
      ...
      environment,
      created_at: new Date().toISOString(),
      expires_at: plan === 'FREE' ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString() : null,
      last_used: null,
      active: true,
    ...
    return {
      ...
      plan_details: {
        ...
        requests_per_minute: PLANS[plan].requests_per_minute,
        modules_allowed: PLANS[plan].modules_allowed,
      },
  ...
  /**
   * Validate an API key and check permissions for a specific module
   */
  validate(apiKey, module = null) {
    ...
    if (module) {
      ...
      if (!hasAccess) {
        return {
          valid: false,
          error: `Module '${module}' is not available on the ${plan.name} plan`,
          code: 'INSUFFICIENT_PLAN',
          upgrade_url: 'https://www.webagentbridge.com/#pricing',
          current_plan: plan.name,
          required_plan: this._getMinPlanForModule(module),
        };
      }
    }
    ...
    if (!rateCheck.allowed) {
      return {
        ...
        retry_after_seconds: rateCheck.retry_after,
    ...
    return {
      ...
      usage: {
        today: usage.today + 1,
        limit_today: plan.requests_per_day,
        remaining_today: plan.requests_per_day - usage.today - 1,
      },
    };
  }
  ...
  revoke(apiKey, reason = 'user_request') {
    ...
    return { success: true, message: 'Key revoked successfully', revoked_at: record.revoked_at };
  }

  /**
   * Rotate an API key (revoke old, issue new with same plan)
   */
  rotate(oldKey) {
    ...
    const newKey = this.generateKey({
      ...
      email: record.email,
      environment: record.environment,
      metadata: { ...record.metadata, rotated_from: record.key_id },
    });
    return { success: true, old_key_id: record.key_id, ...newKey };
  }
  ...
  getUsage(apiKey) {
    ...
    return {
      ...
      total: usage.total,
      limit_per_day: plan.requests_per_day,
      limit_per_minute: plan.requests_per_minute,
      remaining_today: Math.max(0, plan.requests_per_day - usage.today),
  ...
  listKeys(adminKey) {
    ...
    if (!adminRecord || adminRecord.plan !== 'INTERNAL') {
      return { error: 'Admin access required' };
    }
    return {
      total: keyStore.size,
      keys: Array.from(keyStore.values()).map(r => ({
        key_id: r.key_id,
        plan: r.plan,
        owner: r.owner,
        email: r.email,
  ...
  _checkRateLimit(apiKey, plan) {
    ...
    if (rl.count >= limit) {
      const retryAfter = Math.ceil((rl.windowStart + window - now) / 1000);
      return {
        allowed: false,
        retry_after: retryAfter,
        limit,
        reset_at: new Date(rl.windowStart + window).toISOString(),
      };
    }
  ...
  _getMinPlanForModule(module) {
    const moduleToMinPlan = {
      'agent-firewall': 'PRO',
      'notary': 'BUSINESS',
      'dark-pattern': 'FREE',
      'collective-bargaining': 'PRO',
      'gov-intelligence': 'BUSINESS',
      'price-time-machine': 'FREE',
      'neural': 'PRO',
      'protocol': 'FREE',
      'bounty': 'FREE',
      'affiliate': 'PRO',
    };
    return moduleToMinPlan[module] || 'PRO';
  }
