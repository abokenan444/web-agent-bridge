...
(function(global) {
  ...
  WABInstance.prototype.negotiate = function(agentId, proposal) {
    var self = this;
    if (!this.serverUrl) {
      return Promise.resolve({ error: 'Negotiation requires a server URL' });
    }
    return fetch(this.serverUrl + '/api/sovereign/negotiation/sessions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ siteId: self.name, agentId: agentId })
    }).then(function(r) { return r.json(); })
  ...
  WABInstance.prototype.submitAttestation = function(agentId, targetSiteId, score, evidence) {
    if (!this.serverUrl) {
  ...
  WABInstance.prototype.verifyPrice = function(productId, domPrice, visionPrice) {
    if (!this.serverUrl) {
  ...
  var WAB = {
    ...
    verifyPrice: function(productId, domPrice, visionPrice) {
      if (WAB._instance) return WAB._instance.verifyPrice(productId, domPrice, visionPrice);
      return Promise.resolve({ error: 'WAB not initialized' });
    },
